...
const { handleSePayWebhook } = require('./services/sepayWebhook');

const app = express();
app.use(express.json());

app.get('/health', (req, res) => {
    res.status(200).json({ success: true, service: 'telegram-shop-bot' });
});

app.post('/sepay/webhook', (req, res) => {
    handleSePayWebhook(bot, req, res);
});

app.listen(config.WEBHOOK_PORT, () => {
    console.log(`🌐 Webhook server đang chạy tại port ${config.WEBHOOK_PORT}`);
});
