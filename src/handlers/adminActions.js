...
module.exports = (bot) => {
    ...
    if (!order) {
        return ctx.replyWithHTML(`❌ Không tìm thấy đơn #${orderId}`);
    }

    if (order.status !== 'paid') {
        return ctx.replyWithHTML(
        `❌ Đơn <b>#${orderId}</b> chưa được SePay xác nhận thanh toán.<br>` +
        `Status hiện tại: <code>${order.status}</code>`
        );
    }
    ...
    if (result.success) {
        ctx.replyWithHTML(`✅ Đơn <b>#${orderId}</b> đã giao hàng thành công!`);
    } else {
    ...
    bot.on('text', async (ctx, next) => {
        ...
        if (state.action === 'deliver_order') {
            ...
            if (state.action === 'deliver_order') {
                const order = orderService.getById(state.orderId);

                if (!order) {
                    delete adminState[ctx.from.id];
                    return ctx.replyWithHTML(`❌ Không tìm thấy đơn #${state.orderId}`);
                }

                if (order.status !== 'paid') {
                    delete adminState[ctx.from.id];
                    return ctx.replyWithHTML(
                    `❌ Đơn #${state.orderId} chưa được xác nhận thanh toán, không thể giao tay.`
                    );
                }
                ...
                const delivered = orderService.manualDeliver(state.orderId);
                if (!delivered?.success) {
                    return ctx.replyWithHTML(`❌ ${delivered.error}`);
                }
            }
