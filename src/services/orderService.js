...
const orderService = {
  ...
  // Chỉ cho giao khi đơn đã được xác nhận thanh toán
  if (order.status !== 'paid') {
    return {
      success: false,
      error: `Đơn #${orderId} chưa được xác nhận thanh toán (status=${order.status})`
    };
  }
  ...
  if (order.status !== 'paid') {
    return { success: false, error: 'Chỉ được giao tay cho đơn đã paid' };
  }
