    ...
    try {
        ...
        if (realStock.length >= order.quantity) {
            ...
            if (!result.success) {
                ...
                // QUAN TRỌNG:
                // Trả lỗi thật để SePay biết webhook chưa xử lý xong
            ...
            // Chỉ tới đây mới coi là thành công thật
            await safeSend(
                bot,
                config.ADMIN_ID,
                ...
                `Đơn #${order.id}\n` +
                `Mã CK: <code>${paymentCode}</code>\n` +
                `Số tiền: <b>${transferAmount}</b>\n` +
                `Ref: <code>${payload.referenceCode || ''}</code>`
            );
